...
async function getSwapFromRoute(ctx, params, opts = fetcher_1.PREFER_CACHE, txBuilder = new common_sdk_1.TransactionBuilder(ctx.connection, ctx.wallet, (0, txn_utils_1.contextOptionsToBuilderOptions)(ctx.opts))) {
    ...
    const ataInstructionMap = await cachedResolveOrCreateNonNativeATAs(wallet, requiredAtas, (keys) => {
 ...
 * @returns
 */
async function cachedResolveOrCreateNonNativeATAs(ownerAddress, tokenMints, getTokenAccounts, payer = ownerAddress, allowPDAOwnerAddress = false) {
    ...
    tokenAccounts.forEach((tokenAccount, index) => {
        ...
        if (tokenAccount) {
            ...
            const createAtaInstruction = (0, spl_token_1.createAssociatedTokenAccountInstruction)(payer, ataAddress, ownerAddress, tokenMintArray[index]);
            resolvedInstruction = {
                address: ataAddress,
